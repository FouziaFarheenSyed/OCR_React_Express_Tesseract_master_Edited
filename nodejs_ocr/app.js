...
const capturedImage = async (req, res, next) => {
    try {
        ...
        const imagePath = './storage/ocr_image.jpeg';
        ...
        // Use local file path instead of URL for Tesseract
        Tesseract.recognize(
            imagePath,  // Use local file path
            'eng',
            { logger: m => console.log(m) }
        )
        .then(({ data: { text } }) => {
        ...
        .catch(err => {
            ...
            return res.status(500).send({ error: 'OCR processing failed' });
        })

    } catch (e) {
        console.error('Error in capturedImage:', e);
        next(e);
    }
}
